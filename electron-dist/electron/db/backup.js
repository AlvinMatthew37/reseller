...
function getBackupDefaultName(prefix, extension) {
...
function restoreBackupPayload(payload) {
    ...
    db.transaction(() => {
        ...
        for (const scheduledVendor of payload.scheduledVendors) {
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run(scheduledVendor.id, scheduledVendor.schedule_day_id, scheduledVendor.vendor_id, scheduledVendor.vendor_note, scheduledVendor.sort_order);
        }
    })();
}
...
export async function exportJsonBackup(browserWindow) {
...
export async function importJsonBackup(browserWindow) {
    const currentWindow = assertBrowserWindow(browserWindow);
...
export async function exportVendorsCsv(browserWindow) {
    const currentWindow = assertBrowserWindow(browserWindow);
    const db = getDatabase();
    const rows = db
        .prepare(`
...
export async function exportMenusCsv(browserWindow) {
    const currentWindow = assertBrowserWindow(browserWindow);
    const db = getDatabase();
    const rows = db
        .prepare(`
