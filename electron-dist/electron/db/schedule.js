...
function fromDateOnly(dateText) {
...
function ensureScheduleDay(date) {
...
function getLatestScheduledDateByVendorIds(vendorIds) {
...
function computeBaruJual(latestScheduledDate) {
...
function getScheduledVendorsForDay(scheduleDayId) {
    const rows = getDatabase()
        .prepare(`
        SELECT
...
export function getScheduleDayViewByDate(date) {
...
export function getWeekSchedule(anchorDate) {
    const weekStart = startOfWeekMonday(fromDateOnly(anchorDate));
    return Array.from({ length: 7 }, (_, index) => getScheduleDayViewByDate(toDateOnly(addDays(weekStart, index))));
}
export function getScheduleRange(startDate, endDate) {
    const days = [];
...
export function upsertDayNote(date, generalNote) {
    const scheduleDay = ensureScheduleDay(date);
    getDatabase()
        .prepare(`
...
export function addVendorToDay(date, vendorId) {
    const scheduleDay = ensureScheduleDay(date);
    const db = getDatabase();
    const existing = db
        .prepare(`
...
export function updateScheduledVendorNote(scheduledVendorId, vendorNote) {
    const db = getDatabase();
    ...
    const dateRow = db
        ...
        WHERE id = ?
      `)
        .get(scheduledVendor.schedule_day_id);
    return getScheduleDayViewByDate(dateRow.date);
}
export function removeVendorFromDay(scheduledVendorId) {
    const db = getDatabase();
    const row = db
        .prepare(`
        SELECT sd.date
